...
An EOA has no code of its own (its `codeHash` field is a fixed, universal empty-code hash) and is controlled entirely by whoever holds its corresponding private key, the same [ECDSA](../cryptography/ecdsa.md) key pair mechanics covered in Cryptography, using the same [secp256k1](../cryptography/secp256k1.md) curve Bitcoin uses, though Ethereum derives its addresses differently (see below) and, as of this writing, uses ECDSA exclusively for standard account signatures rather than offering a Schnorr-based alternative the way post-Taproot Bitcoin does.

Since the Pectra upgrade (May 2025), [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702) lets an EOA temporarily or persistently *delegate* execution to a contract's logic, a query for that EOA's code now returns a short designator pointing at the delegate, rather than the universal empty-code hash. The account remains an EOA at the protocol level (still controlled by its private key, still the only kind of account that can originate a transaction). The delegation only changes what happens when *other* contracts call into it, letting an EOA gain contract-like behavior (batched calls, custom validation logic) without becoming a contract account in the [Ethereum Accounts](./accounts.md#the-two-kinds-of-account) sense. This means "has no code" is now a slightly imprecise shorthand for "has no code of its own"; see [examples/wallet-stats](../../examples/wallet-stats/) for a real, live example of detecting this distinction correctly.
