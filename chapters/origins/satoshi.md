...
- **Coin holdings.** On-chain analysis (not a statement from Satoshi) has identified a large cluster of addresses, consistent with the mining reward pattern from Bitcoin's earliest blocks, that has never moved funds since being mined. Estimates of this cluster's size (commonly cited around one million bitcoin) come from blockchain analysis firms and independent researchers, not from any confirmed statement by Satoshi. TODO: cite a specific, methodologically transparent on-chain analysis (e.g., Sergio Demian Lerner's "Patoshi pattern" research) rather than restating a round number as settled fact.
